...
int main(int argc, char** argv){
	...
	// OPEN CL HOST CODE AREA START //
	cl_int err; 	// FOR ERROR CHECKING

	std::vector<cl::Device> devices = xcl::get_xil_devices();	// GET LIST OF XILINX DEVICES
	cl::Device device = devices[0];								// GET FPGA IN USE BY SYSTEM

	OCL_CHECK(err, cl::Context context(device, NULL, NULL, NULL, &err));	// GET CONTEXT OF FPGA
    OCL_CHECK(err, cl::CommandQueue q(context, device, CL_QUEUE_PROFILING_ENABLE, &err));	// GET COMMAND QUEUE OF FPGA
    OCL_CHECK(err, std::string device_name = device.getInfo<CL_DEVICE_NAME>(&err)); 	// GET DEVICE INFO
    ...
    OCL_CHECK(err, cl::Kernel disassemble(program,"vadd", &err));
	...
	for(int i = 0; i < shnum; i++){
		...
		if((sh_flags & 0x4) != 0){
			...
			for(offset = 0; offset < sh_size; offset += DATA_SIZE){
			    ...
			    OCL_CHECK(err, cl::Buffer buffer_instructions   (context,CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
			            size, buffer + offset, &err));
			    OCL_CHECK(err, cl::Buffer output_buffer(context,CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, 
			            DATA_SIZE, out, &err));

				OCL_CHECK(err, err = q.enqueueMigrateMemObjects({instructions},0/* 0 means from host*/));

				OCL_CHECK(err, err = disassemble.setArg(0, buffer_instructions));
			    OCL_CHECK(err, err = disassemble.setArg(1, output_buffer));
			    OCL_CHECK(err, err = disassemble.setArg(2, size));
			    OCL_CHECK(err, err = disassemble.setArg(3, size_out));
